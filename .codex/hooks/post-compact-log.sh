...
#
# DESCRIPTION
#   Append a concise Codex/Claude session summary to a daily log file.
#   For Claude PostCompact payloads, this reads compact_summary. For Codex Stop
#   hooks, which do not currently expose a PostCompact event, this falls back to
...
log_dir="${CODEX_COMPACT_LOG_DIR:-/Users/nickolas/nickolashkraus/agent-os/master/notes/daily/logs}"
...
if [ -z "${compact_summary}" ] && [ "${hook_event_name}" = "Stop" ]; then
  compact_summary="${last_assistant_message}"
fi
