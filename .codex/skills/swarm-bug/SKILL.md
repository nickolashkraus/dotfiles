...
principles, then `~/.codex/skills/swarm-bug/PRINCIPLES.md` for bug-specific
lessons.
...
flag. The bug report is either a file path or inline text. If it is a URL
(Slack, Linear, GitHub), fetch its contents. If it is a file path, read it. If
it is inline text, use it directly.
...
not sequential). Investigation agents read the codebase but do not write code,
so they run against the main working directory (no code worktree needed).
...
`~/.codex/skills/swarm-core/prompts/convergence.md`. Spawn all four agents
...
`~/.codex/skills/swarm-core/prompts/synthesis-review.md`. Spawn the four agents
again (parallel). For each, substitute `{own_findings_path}` with the path to
the agent's `investigation.md`, `{synthesis_path}` with the path to
...
issue slug prefix, summary of the fix), push the branch, and create the PR
using `gh pr create`.
