...
compliance and code correctness.
...
2. For each work item, check that the implementation matches the spec's intent,
   not just its letter.
3. Check dependency ordering: do work items that must complete before others
   actually do so?
4. Flag any deviations from the spec, intentional or accidental.
5. Check the implementation against the codebase's existing conventions (error
   handling, logging, naming, layering).
