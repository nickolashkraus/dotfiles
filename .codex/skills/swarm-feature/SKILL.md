...
Parse the user-provided skill input for the feature spec and optional
`--linear` flag. The spec is either a file path or inline text. If it is a URL
(Linear, Notion), fetch its contents. If it is a file path, read it. If it is
...
Spawn all four agents again. Substitute `{own_findings_path}`,
`{other_findings_paths}`, and `{output_file}` (= the agent's
`investigation.md`). Each agent appends under a `## Convergence` section. Write
